...
public class Postman {
	...
	public Postman(List<Integer> list) {
		this(list.get(0));
		makeGraphFromFile(list);
	}

	public void makeGraphFromFile(List<Integer> list) {
	...
	public boolean isConnected() {
		...
		if (graph.getNumOfVertices() > 0) {
			...
			DFS(0);
			return (graph.getNumOfVertices() == visitedVertices.size()) ? true : false;
		}
		return false;
	}
	...
	private void DFS(int v) {
		...
		Iterator<Integer> iterator = graph.getAdjacencyList()[v].listIterator();
	...
	private LinkedList<Integer> findShortestPathBFS(int src, int dst) {
		...
		Map<Integer, Integer> parentVertices = new HashMap<Integer, Integer>();
	...
	public LinkedList<Integer> getHeuristicCycle(int beginVertice) {
		...
		while (true) {
			...
			if (tmp != null) {
				...
				if (previousVertices[tmp] == null || current == start)
	...
	private LinkedList<Integer> leaveLoop(LinkedList<Integer> cycleList, LinkedList<Integer> loop,
		...
		LinkedList<Integer> partList = new LinkedList<Integer>();
		int posInLoop = 0;
		int posInCycle = 0;
		...
		for (int i = posOfStartVertice; i >= 0; --i) {
			if (cycleList.get(i) == loop.get(posInLoop)) {
				posInCycle = i;
				break;
			}
		}
